...
class SubmitMarketingCampaignPostToN8nAction
{
    public function __construct(
        private readonly MarketingCampaignPostMediaPayloadBuilder $builder
    ) {}
    ...
    public function execute(MarketingCampaignPost $post, array $runtimeClientData = []): void
    {
        ...
        try {
            $jobData = DB::transaction(function () use (&$post, $runtimeClientData, $client, $includeLogo, &$logoUrl, &$tempPathToDelete, &$savedToClientLogo, &$clientLogoPathToSave, &$runtimeLogoPath, $campaign) {
                ...
                if ($includeLogo) {
                    if ($client->logo_path) {
                        ...
                        if ($runtimeLogoFile instanceof UploadedFile) {
                            ...
                            if (! empty($runtimeClientData['save_runtime_logo_to_client'])) {
                                ...
                                $logoUrl = URL::temporarySignedRoute(
                                    'media.public',
                                    now()->addMinutes(
            ...
            if ($runtimeLogoPath !== null) {
                Storage::disk('public')->delete($runtimeLogoPath);
            }
            throw $exception;
        }
        ...
        if ($jobData) {
            try {
                ...
                try {
                    DB::transaction(function () use ($post, $jobData, &$fileToDelete) {
                        $lockedPost = MarketingCampaignPost::lockForUpdate()->findOrFail($post->id);

                        if ($lockedPost->n8n_request_id === $jobData['payload']['request_id']
                            && $lockedPost->status->value === MarketingCampaignPostStatus::PendingN8n->value
                        ) {
                            $lockedPost->update([
                    ...
                    Log::error('Impossibile ripristinare il DB dopo il dispatch fallito di n8n', [
                        'post_id' => $post->id,
                ...
                try {
                    ...
                    Log::error('Impossibile rimuovere il file locale temporaneo dopo il dispatch fallito', [
                        'path' => $fileToDelete,
