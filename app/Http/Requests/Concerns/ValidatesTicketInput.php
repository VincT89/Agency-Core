...
trait ValidatesTicketInput
{
    public function rules(): array
    {
        ...
        return [
            ...
            'requested_services' => ['exclude_unless:type,quote', Rule::requiredIf($this->input('type') === 'quote' && !$this->route('ticket')), 'array', 'min:1', 'max:50'],
            'requested_services.*' => ['array:name,description'],
            'requested_services.*.name' => ['required', 'string', 'max:255'],
            'requested_services.*.description' => ['nullable', 'string', 'max:3000'],
