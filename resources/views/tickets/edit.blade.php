<x-app-layout title="Modifica Ticket">
    <x-page-header
        eyebrow="Modulo · Operativo"
        
    >
    <x-slot:title><strong>Modifica</strong> ticket</x-slot:title>
        <x-slot:actions>
            <a href="{{ route('tickets.show', $ticket) }}" class="btn btn-g">← Indietro</a>
        </x-slot:actions>
    </x-page-header>
        ...
        <form action="{{ route('tickets.update', $ticket) }}" method="POST">
            ...
            <div class="form-row">
                <x-form-group label="Tipo" name="type" required>
                    <select name="type" class="form-sel @error('type') is-invalid @enderror">
                        @foreach($types as $t)
                            <option value="{{ $t }}" {{ old('type', $ticket->type) == $t ? 'selected' : '' }}>{{ ucfirst($t) }}</option>
                        @endforeach
                    </select>
                </x-form-group>
            ...
            <div class="form-row">
                <x-form-group label="Stato" name="status" required>
                    <select name="status" class="form-sel @error('status') is-invalid @enderror">
                        @foreach($statuses as $s)
                            <option value="{{ $s }}" {{ old('status', $ticket->status) == $s ? 'selected' : '' }}>{{ (new \App\Models\Ticket(['status' => $s]))->status_label }}</option>
                        @endforeach
                    </select>
                </x-form-group>
                <x-form-group label="Assegnato a" name="assigned_to">
                    <select name="assigned_to" class="form-sel @error('assigned_to') is-invalid @enderror">
                        ...
                        @foreach($users as $u)
                            <option value="{{ $u->id }}" {{ old('assigned_to', $ticket->assigned_to) == $u->id ? 'selected' : '' }}>
                                {{ $u->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="form-row full">
                <x-form-group label="Dettagli" name="description">
                    <textarea name="description" class="form-ta @error('description') is-invalid @enderror" rows="4">{{ old('description', $ticket->description) }}</textarea>
                </x-form-group>
            </div>
